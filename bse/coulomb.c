...
void calc_eh_kernel_cplx(
  ...
  unsigned long         start, end, ncycles;
  unsigned long*        listibs;

  unsigned long         i, j, a, b, ibs, jbs, jgr, jsgr;
  unsigned long         ab, a_st, b_st, i_st, j_st;
  unsigned long         loop_idx;

  const unsigned long   nspngr     = ist->nspinngrid;
  const unsigned long   ngrid      = ist->ngrid;
  const unsigned long   lidx       = ist->lumo_idx;
  ...
  rho = (zomplex *) calloc(ist->ngrid, sizeof(zomplex));
	listibs = (unsigned long *) malloc(ist->n_xton * sizeof(unsigned long));
  ...
  /************************************************************/
    ...
    printf("Number of cycles per rank = %lu\n", ncycles);
  ...
  /*******************************************************************/
    ...
    unsigned long  ai;
    unsigned long  ai_tot    = n_el * n_ho;
    unsigned long  ns_p_rank = ai_tot / odd_size;
    unsigned long  *lista, *listi;
    
    lista   =  (unsigned long *) calloc(ai_tot, sizeof(unsigned long));
    listi   =  (unsigned long *) calloc(ai_tot, sizeof(unsigned long));
...
unsigned long load_coulomb_mat(zomplex* mat, char* fileName, index_st* ist){
  ...
  long line;
  int ieof = 0;
  unsigned long max_a = 0;
  unsigned long a, b, i, j, ibs, jbs;
  unsigned long max_st_num = ist->n_holes*ist->n_holes*ist->n_elecs*ist->n_elecs;
  double tmp_re, tmp_im;
