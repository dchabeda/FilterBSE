...
void diag(const int n, int nthreads, zomplex *mat, double *eval) {
  ...
  char uplo;
  const long long N = n;
  // const long long lwork = 3*n - 1;
  // zomplex *work; // dimension (MAX(1,lwork))
  // double *rwork; //dimension (MAX(1,3*n-2))
  
  // Use multiple threads

  // Allocate memmory for work array
  // rwork = (double *) calloc(lwork, sizeof(double));
  // work =  calloc(lwork, sizeof(zomplex));
  ...
  // LAPACKE_zheev(&jobz, &uplo, &n, &mat[0], &n, &eval[0], &work[0], &lwork, &rwork[0], &info);
