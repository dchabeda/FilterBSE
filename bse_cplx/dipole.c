    ...
    flag_st *flag)
{
  ...
  for (a = lidx; a < lidx + n_el; a++)
  {
    ...
    // spin up part
    l_operator(&Lxpsi[0], &Lypsi[0], &Lzpsi[0], &psi_qp[a_st], g_vecs, grid, ist, par, planfw, planbw, fftwpsi);
    // spin dn part
...
#pragma omp simd safelen(8) aligned(psi_qp, Lxpsi, Lypsi, Lzpsi : BYTE_BOUNDARY) reduction(+ : m_x, m_y, m_z)
      ...
      {
        m_x += conjmul(psi_qp[i_st + jg], Lxpsi[jg]);
        m_y += conjmul(psi_qp[i_st + jg], Lypsi[jg]);
