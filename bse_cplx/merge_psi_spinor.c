...
int main(int argc, char *argv[])
{
  if (argc != 10)
  {
    printf("Usage: merge_psi_spinor.x homoeval homopsi lumoeval lumopsi ngrid nhomo nlumo deps fermiE\n");
    exit(EXIT_FAILURE);
  }
  ...
  double fermiEnergy = atof(argv[9]);
  ...
  {
    if (i >= nlines_ho)
      break; // break the loop if we iterate past the number of lines in the file
    ieof = fscanf(pevalhomo, "%ld %lg %lg", &a, &evalloc, &deloc);
    if (deloc < deps && evalloc < fermiEnergy)
      ihomo = i; // Grabs the last state with small variance before exceeding the fermiEnergy
    if (evalloc > fermiEnergy)
      break; // do not search through the electron states
  }
  fclose(pevalhomo);
