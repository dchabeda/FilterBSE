  ...
  ********************************************************************/
  ...
  nvtxRangePushA("Calc qp hole spin mat elems");
	for(i = 0; i < ist->n_holes; i++){
      ...
      #pragma omp parallel for simd reduction(+:sx_re, sx_im, sy_re, sy_im, sz_re, sz_im)
        ...
        // printf("\n%lu\n", jgr);
        // printf("psi[jdn] = %g %g psi[iup] = %g %g\n", psi_jdr, psi_jdi, psi_iur, psi_iui );
        // printf("conjmul  = %g %g\n", psi_jdr * psi_iur + psi_jdi * psi_iui, psi_jdr * psi_iui - psi_jdi * psi_iur);
        // printf("sx       = %g %g\n", sx_re, sx_im );
        // <j|r,up> * <r,dn|i>
        sx_re += psi_jur * psi_idr + psi_jui * psi_idi;
        sx_im += psi_jur * psi_idi - psi_jui * psi_idr;
        // printf("psi[jup] = %g %g psi[idn] = %g %g\n", psi_jur, psi_jui, psi_idr, psi_idi );
        // printf("conjmul  = %g %g\n", psi_jur * psi_idr + psi_jui * psi_idi, psi_jur * psi_idi - psi_jui * psi_idr);
        // printf("sx       = %g %g\n", sx_re, sx_im );
      ...
      printf("sx_mom = %g %g\n", s_mom[i*ist->n_holes+j].x_re, s_mom[i*ist->n_holes+j].x_im);
      exit(0);
      nvtxRangePop();
		}
	}
...
void calc_ang_mom_mtrx(
	...
	zomplex *restrict       l2_mom,
	zomplex *restrict       LdotS,
	double *restrict        psi_qp,
	grid_st*                grid,
	index_st*               ist,
	par_st*                 par){
  ...
  nvtxRangePushA("Calc qp hole L elems");
    ...
    nvtxRangePop();
		#pragma omp parallel for private(j_st, jgr, jsgr, jgur, jgui, jgdr, jgdi)
		for (j = i; j < n_ho; j++){
  ...
  nvtxRangePushA("Calc qp elec L elems");
    ...
    nvtxRangePushA("Compute L by FFTs");
		...
		#pragma omp parallel for private(b, b_st, jgr, jsgr, jgur, jgui, jgdr, jgdi, idx)
