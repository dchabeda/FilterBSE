    ...
    for (a = lidx; a < lidx + n_el; a++) {
      ...
      for (jz = 0; jz < ist->nz; jz++) {
        ...
        for (jy = 0; jy < ist->ny; jy++) {
          ...
          #pragma omp simd safelen(8) aligned(psi_qp, grid->x: BYTE_BOUNDARY) reduction(+: mu_x_re, mu_x_im, mu_y_re, mu_y_im, mu_z_re, mu_z_im)
          for (jx = 0; jx < ist->nx; jx++) { 
            x = grid->x[jx];
            jg = jyz + jx;

            // Index this iteration of the loop
            // __builtin_prefetch(&ist->jgur[jg + 8], 0, 1);
