...
int main(int argc, char *argv[])
{
  ...
  {
    fprintf(stdout, "******************************************************************************\n");
    fprintf(stdout, "\nRUNNING PROGRAM: BETHE-SALPETHER (real-valued version)\n");
  ...
  if (argv[1] != NULL)
  {
    flag.initUnsafe = (int)strtol(argv[1], &endptr, 10);
  }

  mod_init(
      &psi_qp, &eig_vals, &sigma_E, &R, &grid,
  ...
  {
    printf("No excitonic properties requested. flag->noCalcExciton = %d!\n", flag.noCalcExciton);
    printf("Job terminating at | %s\n", get_time());
    exit(EXIT_SUCCESS);
  }
  // /************************************************************/
  // /**********************    MOD POT     **********************/
