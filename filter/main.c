...
int main(int argc, char *argv[])
{
  ...
  char *file_name;
  file_name = malloc(9 * sizeof(file_name[0]));
  ...
  {
    if ((SO_projectors = (double *)calloc(PROJ_LEN * ist.nproj, sizeof(double))) == NULL)
    {
      nerror("mem_SO_projector");
    }
  }
  ...
  case 0:
    ...
    //
    //
    run_filter_cycles(psitot, pot_local, atom, nlc, nl, ksqr, an, zn, ene_targets, &grid, &ist, &par, &flag, &parallel);
    //
    //
    //
  ...
  case 1:
    if (1 == flag.restartFromOrtho)
    {
      ...
      fflush(stdout);
      printf("Size of psitot array = %.2g GB\n", (double)psitot_size / 1024 / 1024 / 1024);
      fflush(stdout);
      ...
      for (jstate = 0; jstate < 5; jstate++)
      {
        ...
        for (jgrid = 0; jgrid < ist.ngrid; jgrid++)
        {
          ...
          if (1 == flag.isComplex)
          {
            ...
            if (sgn_val < psitot[state_idx + jgrid_imag])
              sgn_val = psitot[state_idx + jgrid_imag];
          }
          rho[jgrid] *= sign(sgn_val);
        }
  ...
  case 2:
    ...
    for (i = ieof = 0; ieof != EOF; i++)
    {
      ...
      if (deloc < par.sigma_E_cut && evalloc < par.fermi_E)
        ist.homo_idx = i;
      ...
      else
      {
        ...
        for (i = 0; (i < ist.total_lumo) && (i < ist.ncubes); i++)
        {
          ...
          for (jgrid = 0; jgrid < ist.ngrid; jgrid++)
          {
            ...
            if (1 == flag.isComplex)
              sgn_val.im = psitot[ist.complex_idx * (ist.lumo_idx + i) * ist.nspinngrid + jgrid_imag];
            rho[jgrid] = sign(sgn_val.re + sgn_val.im) * (sqr(sgn_val.re) + sqr(sgn_val.im));
          }
          write_cube_file(rho, &grid, str);
    ...
    printf("Total job CPU time (sec) %.4g, wall run time (sec) %.4g",
           ((double)end_clock - (double)start_clock) / (double)(CLOCKS_PER_SEC), (double)end_time - (double)start_time);
    fflush(0);
    write_separation(stdout, bottom);
