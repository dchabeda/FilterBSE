...
int main(int argc, char const *argv[]){
	...
	double dx, dy, dz, *gridX, *gridY, *gridZ, prefactor, r, r_scaled, x,y,z;
	long flags = 0;
	FILE *pf;
...
//initialize grid and par and ist structs and fftw vars
	...
	/*** the grid in the x, y, and z directions ***/
  	...
  	for (jx = 0, dx = par.xmin+0.5*par.dx; jx < ist.nx; jx++, dx += par.dx) gridX[jx] = dx;
  	for (jy = 0, dy = par.ymin+0.5*par.dy; jy < ist.ny; jy++, dy += par.dy) gridY[jy] = dy;
  	for (jz = 0, dz = par.zmin+0.5*par.dz; jz < ist.nz; jz++, dz += par.dz) gridZ[jz] = dz;
	...
	//fclose(pf);
	normalize_all( psitot , par.dv,ist.mn_states_tot, ist.nspinngrid, ist.nthreads);

	calc_angular_exp(psitot, gridX, gridY, gridZ, planfw, planbw, fftwpsi, ist, par);
