...
void diag_H(
  ...
  ********************************************************************/
  ...
  if (1 == flag->isComplex){
    H_z = (zomplex*)calloc(ist->mn_states_tot*ist->mn_states_tot,sizeof(zomplex));
    work_z = (zomplex*)calloc(lwk,sizeof(zomplex));
  ...
  for (ims = 0; ims < ist->mn_states_tot; ims++){
    ...
    for (jms = 0; jms <= ims; jms++){
      ...
      if (1 == flag->isComplex){
        ...
        H_z[jms*ist->mn_states_tot + ims].im *= -1;
      }
    }
  ...
  for (ims = 0; ims < ist->mn_states_tot; ims++){
    for (jms = 0; jms < ist->mn_states_tot; jms++){
      ...
      if (1 == flag->isComplex){
        fprintf(pg, "%lg+i%lg ", H_z[ims*ist->mn_states_tot + jms].re, H_z[ims*ist->mn_states_tot + jms].im);
      }
    }
    fprintf(pg, "\n");
  }
  ...
  if (0 == flag->isComplex){
    char jobu = 'O'; 
    char jobvt = 'N';
    dsyev_(&jobu, &jobvt, &mn_states_tot, &H[0], &mn_states_tot, &eval[0], &work[0], &lwk, &info , 1, 1);
    if (info) { 
  ...
  if (1 == flag->isComplex){
    char jobu = 'O'; 
    char jobvt = 'N';
    zheev_(&jobu, &jobvt, &mn_states_tot, &H_z[0], &mn_states_tot, &eval[0], &work_z[0], &lwk, &(rwork[0]), &info , 1, 1);
    if (info){
  ...
  for (jgrid = 0; jgrid < ist->nspinngrid; jgrid++){
    ...
    for (jms = 0; jms < ist->mn_states_tot; jms++){
      ...
      for (ims = 0; ims < ist->mn_states_tot; ims++){
        ...
        if (1 == flag->isComplex){
          sumre += (H_z[jms*ist->mn_states_tot+ims].re * tpsi[ims].re + H_z[jms*ist->mn_states_tot+ims].im * tpsi[ims].im);
          sumim += (H_z[jms*ist->mn_states_tot+ims].re * tpsi[ims].im - H_z[jms*ist->mn_states_tot+ims].im * tpsi[ims].re);
        }
      }
...
zomplex dotp(zomplex *psi, double *psitot,long m,long ngrid,double dv){
  ...
  for (jgrid = 0; jgrid < ngrid; jgrid++){
    ...
    sum.re += (psi[jgrid].re * psitot[2*m*ngrid + jgrid_real] + psi[jgrid].im * psitot[2*m*ngrid + jgrid_imag]);
    sum.im += (psi[jgrid].re * psitot[2*m*ngrid + jgrid_imag] - psi[jgrid].im * psitot[2*m*ngrid + jgrid_real]);
  }
  sum.re *= dv;
  sum.im *= dv;
