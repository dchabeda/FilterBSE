...
void calc_angular_exp(double *psitot, grid_st *grid, int start, int stop, index_st *ist, par_st *par, flag_st *flag, parallel_st *parallel)
{
  ...
  if ((Jxpsi = (zomplex *)calloc(ist->nspinngrid, sizeof(zomplex))) == NULL)
    nerror("Jxpsi");
  if ((Jypsi = (zomplex *)calloc(ist->nspinngrid, sizeof(zomplex))) == NULL)
    nerror("Jypsi");
  if ((Jzpsi = (zomplex *)calloc(ist->nspinngrid, sizeof(zomplex))) == NULL)
    nerror("Jzpsi");
...
#pragma omp parallel for reduction(+ : Jxexp, Jxsqr, Jyexp, Jysqr, Jzexp, Jzsqr, Jsqrexp, norm)
    for (i = 0; i < ist->nspinngrid; i++)
    {
      ...
      Jsqrexp += (Jxpsi[i].re * Jxpsi[i].re) + (Jxpsi[i].im * Jxpsi[i].im) + (Jypsi[i].re * Jypsi[i].re) + (Jypsi[i].im * Jypsi[i].im) + (Jzpsi[i].re * Jzpsi[i].re) + (Jzpsi[i].im * Jzpsi[i].im);
    ...
    parallel_st *parallel)
{
  ...
  center.y = y * par->dv;
  center.z = z * par->dv;
  printf("Center of psi distribution: <%f, %f, %f>\n", center.x, center.y, center.z);
  ...
  if ((kx = (double *)calloc(grid->nx, sizeof(double))) == NULL)
    nerror("kx");
  if ((ky = (double *)calloc(grid->ny, sizeof(double))) == NULL)
    nerror("ky");
  if ((kz = (double *)calloc(grid->nz, sizeof(double))) == NULL)
    nerror("kz");

  /***initializing the k vectors ***/
  init_kvec(kx, grid->nx, grid->dkx, (1.0 / (double)grid->ngrid));
  init_kvec(ky, grid->ny, grid->dky, (1.0 / (double)grid->ngrid));
  init_kvec(kz, grid->nz, grid->dkz, (1.0 / (double)grid->ngrid));
  ...
  for (jz = 0; jz < grid->nz; jz++)
  {
    ...
    for (jy = 0; jy < grid->ny; jy++)
    {
      ...
      for (jx = 0; jx < grid->nx; jx++)
      {
        ...
        gradx.re = Lxpsi[jxyz].re;
        gradx.im = Lxpsi[jxyz].im;
        grady.re = Lypsi[jxyz].re;
        grady.im = Lypsi[jxyz].im;
        gradz.re = Lzpsi[jxyz].re;
        gradz.im = Lzpsi[jxyz].im;
