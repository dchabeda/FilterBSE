...
void gen_newton_coeff(zomplex *an, double *samp, double *ene_targets, index_st *ist, par_st *par, parallel_st *parallel)
{
   ...
   * polynomial approximation of Gaussian filter functions. A total  *
...
#pragma omp parallel for private(ie, i, j, res, sumre, sumim)
...
void read_newton_coeff(zomplex *an, double *zn, index_st *ist, par_st *par)
{
  ...
  for (ie = 0; ie < ist->m_states_per_filter; ie++)
  {
    ...
    for (i = 0; i < ist->ncheby; i++)
    {
      fscanf(pf, "%ld %lg %lg", &tmp, &(coeff.re), &(coeff.im));
      an[ie * ist->ncheby + i].re = coeff.re;
      an[ie * ist->ncheby + i].im = coeff.im;
    }
  }
  fclose(pf);
...
double samp_points_ashkenazy(zomplex *point, double min, double max, long ncheby)
{
  ...
  for (j = 0; j < nc3; j++)
    samp[j].re = samp[j].im = 0.0;
  for (j = 0; j < nc3 - imfrac; j++)
    samp[j].re = min + (double)(j)*dsre;
  for (j = nc3 - imfrac; j < nc3; j++)
    samp[j].im = minim + (double)(j - nc3 + imfrac) * dsim;
  ...
  for (fk = 1.0, j = 0; j < ncheby; j++)
    fk *= (sqr(point[j].re) + sqr(point[j].im));
  fk = sqrt(fk);
