...
void print_input_state(FILE *pf, flag_st *flag, grid_st *grid, par_st *par, index_st *ist, parallel_st *parallel)
{
  ...
  {
    fprintf(stderr, "\tInvalid centerConf parameter!\n");
    exit(EXIT_FAILURE);
  }
  // ****** ****** ****** ****** ****** ******
  ...
  fprintf(pf, "\t--------------------------\n");
  if (1 == flag->useStrain)
    fprintf(pf, "\tStrain dependent pseudopotential will be calculated\n");
  else
    fprintf(pf, "\tStrain dependent pseudopotential will NOT be calculated\n");
  if (1 == flag->interpolatePot)
    fprintf(pf, "\tPseudopotential parameters will be interpolated based on NC geometry\n");
  else
    fprintf(pf, "\tPseudopotential parameters will not be interpolated\n");
  if (1 == flag->LR)
    fprintf(pf, "\tPseudopotentials are long ranged.\n");
  else
    fprintf(pf, "\tPseudopotentials are short ranged.\n");
  fprintf(pf, "\tCrystal structure: %s\n", par->crystal_structure);
  fprintf(pf, "\tOutmost material: %s\n", par->outmost_material);
  if (par->scale_surface_Cs != 1.0)
    fprintf(pf, "\tLong range component of surface Cs atom potentials scaled by %lg\n", par->scale_surface_Cs);
  else
    fprintf(pf, "\tSurface Cs atoms NOT rescaled for charge balancing\n");
  if (flag->readProj == 1)
    fprintf(pf, "\tProjector functions will be read from files\n");
  else
    fprintf(pf, "\tProjector functions will be generated on the fly\n");
  ...
  {
    fprintf(pf, "\tRandom seed will be generated based on clock at runtime\n");
  }
  ...
  else if (0 == flag->approxEnergyRange)
  {
    fprintf(pf, "\tFull energy range of Hamiltonian will be calculated\n");
  }
  else
  {
    fprintf(pf, "Corrupted approxEnergyRange %d\n", flag->approxEnergyRange);
    exit(EXIT_FAILURE);
  }
  ...
  if (flag->useSpinors == 1)
    fprintf(pf, "\tSpinor wavefunctions turned ON!\n");
  else
    fprintf(pf, "\tSpinor wavefunctions turned OFF!\n");

  if (flag->SO == 1)
    fprintf(pf, "\tSpin-orbit coupling is ON!\n");
  else
    fprintf(pf, "\tSpin-orbit coupling is OFF!\n");

  if (flag->NL == 1)
    fprintf(pf, "\tNon-local potential is ON!\n");
  else
  ...
  if (0 == flag->isComplex)
    fprintf(pf, "\tWavefunctions are REAL valued!\n\t  complex_idx = %d\n", ist->complex_idx);
  else if (1 == flag->isComplex)
    fprintf(pf, "\tWavefunctions are COMPLEX valued! complex_idx = %d\n", ist->complex_idx);

  if (1 == par->t_rev_factor)
    fprintf(pf, "\tWavefunctions will NOT be time reversed before ortho!\n\t  t_rev_factor = %d\n", par->t_rev_factor);
  else if (2 == par->t_rev_factor)
    fprintf(pf, "\tWavefunctions will be time reversed before ortho!\n\t t_rev_factor = %d\n", par->t_rev_factor);
  ...
  if (flag->calcPotOverlap == 1)
    fprintf(pf, "\tMatrix elements of pseudopotential will be calculated\n");
  else
    fprintf(pf, "\tMatrix elements of the potential will not be calculated\n");

  if (flag->getAllStates == 1)
    fprintf(pf, "\tAll eigenstates will be saved to disk regardless of variance\n");
  else
    fprintf(pf, "\tEigenstates with variance above %lg a.u. will not be saved to disk\n", par->sigma_E_cut);

  if (flag->timeHamiltonian == 1)
    fprintf(pf, "\tHamiltonian timing specs will be printed\n");
  else
    fprintf(pf, "\tHamiltonian timing specs will not be printed\n");

  if (flag->printCubes == 1)
    fprintf(pf, "\tCube files of top %d states (sigma_E_cut < %lg) in VB & CB will be printed\n", ist->ncubes, par->sigma_E_cut);
  else
    fprintf(pf, "\tCube files will not be printed\n");

  if (flag->calcSpinAngStat == 1)
    fprintf(pf, "\tSpin and Ang. Mom. statistics will be computed\n");
  else
    fprintf(pf, "\tSpin and Ang. Mom. statistics will not be computed\n");
  ...
  if (flag->retryFilter == 1)
    fprintf(pf, "\tFilter will restart if no eigenstates acquired after diag\n");
  else
    fprintf(pf, "\tFilter will NOT restart if no eigenstates obtained after diag.\n");
  if (flag->saveCheckpoints == 1)
    fprintf(pf, "\tFilter will save checkpoints along the job run\n");
  else
    fprintf(pf, "\tNo checkpoint saves requested\n");
  if (flag->restartFromCheckpoint > -1)
    fprintf(pf, "\tFilter will restart from checkpoint %d\n", flag->restartFromCheckpoint);
  else
    fprintf(pf, "\tNo checkpoint specified for restart. Job will run in normal sequence.\n");
  if (flag->restartFromOrtho == 1)
    fprintf(pf, "\tFilter will restart from orthogonalization step!\n");
  if (flag->restartFromSigma == 1)
    fprintf(pf, "\tFilter will restart from sigma step!\n");
                    ...
                    nlc_st *nlc, grid_st *grid, index_st *ist, par_st *par, flag_st *flag, parallel_st *parallel)
{
  ...
  {
    fprintf(pf, "%ld ", ist->atom_types[j]);
  }
...
void save_output(char *file_name, double *psitot, double *eig_vals, double *sigma_E, xyz_st *R, grid_st *grid, index_st *ist, par_st *par, flag_st *flag, parallel_st *parallel)
{
  ...
  // Save parameters struct, par
  fprintf(pf, "%lg %lg\n", par->KE_max, par->fermi_E);

  // Save job flags struct, flag
  fprintf(pf, "%d %d %d %d %d\n", flag->SO, flag->NL, flag->LR, flag->useSpinors, flag->isComplex);

  // Save atomic configuration, R
  for (j = 0; j < ist->natoms; j++)
  {
    fprintf(pf, "%lg %lg %lg\n", R[j].x, R[j].y, R[j].z);
  }

  // Save grid parameters struct, grid
  fprintf(pf, "%lg %lg %lg %lg %lg %lg %lg %lg\n", grid->dx, grid->dy, grid->dz, grid->dr, grid->dv, grid->dkx, grid->dky, grid->dkz);
  fprintf(pf, "%lg %lg %lg %lg %lg %lg\n", grid->xmin, grid->xmax, grid->ymin, grid->ymax, grid->zmin, grid->zmax);
  fprintf(pf, "%ld %ld %ld\n", grid->nx, grid->ny, grid->nz);
  fprintf(pf, "%lg %lg %lg\n", grid->nx_1, grid->ny_1, grid->nz_1);
  ...
  // Save quasiparticle eigenvalues and variances
