...
int main(int argc, char *argv[]){
  ...
  ******************************************************************/  
  ...
  long long psi_rank_sz_k = ist.n_k_pts * ist.psi_rank_size;
  printf("psi_rank_size = %lld\n", ist.psi_rank_size);
  printf("psi_rank_sz_k = %lld\n", psi_rank_sz_k);
  if ((psi_rank = (double *) calloc(psi_rank_sz_k, sizeof(psi_rank[0]))) == NULL){
    if (parallel.mpi_rank == 0) fprintf(stderr,"\nOUT OF MEMORY: psi_rank\n\n"); exit(EXIT_FAILURE);
  }
  // the quasiparticle energies and standard deviations
  if ((eig_vals = (double *) calloc(ist.n_k_pts*par.t_rev_factor*ist.mn_states_tot, sizeof(double))) == NULL){
    if (parallel.mpi_rank == 0) fprintf(stderr, "\nOUT OF MEMORY: eig_vals\n\n"); exit(EXIT_FAILURE);
  }
  if ((sigma_E = (double *) calloc(ist.n_k_pts*par.t_rev_factor*ist.mn_states_tot, sizeof(double))) == NULL){
  ...
  switch (flag.restartFromCheckpoint){
    ...
    case -1:
      ...
      // Get the energy range at the largest k-point value
      k = k_vecs[ist.n_k_pts - 1];
      get_energy_range_k(psi, phi, pot_local, G_vecs, k, &grid, nlc, nl, &ist, &par, &parallel, &flag, planfw, planbw, fftwpsi);
    ...
    case 0:
      ...
      if (1 == flag.timeHamiltonian){
        ...
        // Test Hamiltonian timing at the Gamma point
        k.x = k.y = k.z = k.mag = 0.0;
        time_hamiltonian_k(phi,psi,pot_local,G_vecs,k,&grid,nlc,nl,&ist,&par,&flag,&parallel);
      ...
      run_filter_cycle_k(psi_rank,pot_local,G_vecs,k_vecs,nlc,nl,an,zn,ene_targets,&grid,&ist,&par,&flag,&parallel);
      ...
      // else {
        MPI_Gather(psi_rank,psi_rank_sz_k,MPI_DOUBLE,psitot,psi_rank_sz_k,MPI_DOUBLE,0,MPI_COMM_WORLD);
      // }
      ...
      case 2:
        ...
        calc_sigma_E_k(psitot, pot_local, G_vecs, k_vecs, &grid, nlc, nl, sigma_E, &ist, &par, &flag);
